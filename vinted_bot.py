...
# Store last attachment per channel
last_attachments = {}


# ════════════════════════════════════════════════════════════════════════════
# MAIL 1 — Verkoop bevestiging (/vinted)
# ════════════════════════════════════════════════════════════════════════════
...
def send_sync(to_address: str, subject: str, html: str, attachment_data: bytes = None, attachment_name: str = None):
...
async def send_email(to_address: str, subject: str, html: str, attachment_data: bytes = None, attachment_name: str = None):
    loop = asyncio.get_event_loop()
    await loop.run_in_executor(None, send_sync, to_address, subject, html, attachment_data, attachment_name)


# ════════════════════════════════════════════════════════════════════════════
# MODAL 1 — Vinted verkoop
# ════════════════════════════════════════════════════════════════════════════
...
class VintedModal(discord.ui.Modal, title="Vinted Mail Versturen"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            html = build_vinted_html(seller_val, buyer_val, product_val)
            await send_email(email_val, "You've sold an item on Vinted", html)
...
class LabelModal(discord.ui.Modal, title="Shipping Label Versturen"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Haal laatste bijlage op uit dit channel
        channel_id = interaction.channel_id
        attachment_data = None
        attachment_name = None

        if channel_id in last_attachments:
            att_url, att_name = last_attachments[channel_id]
            try:
        ...
        try:
            ...
            if attachment_name:
                embed.add_field(name="Bijlage", value=f"✅ {attachment_name}", inline=False)
            else:
                embed.add_field(name="Bijlage", value="⚠️ Geen foto gevonden (stuur foto vóór /label)", inline=False)
            embed.set_footer(text="Verstuurd via noreply@vinted-service.nl")
            await interaction.followup.send(embed=embed, ephemeral=True)
        except Exception as e:
            await interaction.followup.send(f"❌ Fout:\n```{e}```", ephemeral=True)
...
async def on_message(message: discord.Message):
    if message.author == client.user:
        return
    # Sla de laatste foto op per channel
